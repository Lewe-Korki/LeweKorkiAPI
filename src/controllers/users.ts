...
class UsersController {
  async getUserInfo(req: Request, res: Response) {
    ...
    try {
      const user = await UsersService.getUserInfo(login)
  ...
  //     const newToken = await UsersService.changeDescription(login, description)
  ...
  //   try {
  //     await jwt.verify(token, config.jwt.secret)
  //     const newToken = await UsersService.changeInterests(login, interests)
  //     res.cookie('jwt', newToken, {
  //       maxAge: Number(config.jwt.expiresIn),
  //     })
  //     res.status(200).send(newToken)
